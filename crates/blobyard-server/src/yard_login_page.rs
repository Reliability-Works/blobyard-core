...
pub(super) fn login(
    ...
    let oidc = if oidc_enabled {
        format!(
            "<form method=\"post\" action=\"/account/yard-oidc/start\"><input type=\"hidden\" name=\"continuation\" value=\"{}\"><button type=\"submit\">Sign in with your identity provider</button></form>",
...
#[allow(
    ...
    reason = "shared by the local-key and sibling OIDC route modules"
)]
pub(crate) fn invalid_link() -> Response<Body> {
    message(
