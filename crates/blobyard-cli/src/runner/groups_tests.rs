...
#[test]
fn normalizes_decomposed_unicode_before_applying_scalar_boundaries() {
    let accepted = format!("{}x", "e\u{301}".repeat(79));
    assert!(matches!(
        group_call(&GroupsCommand::Create(GroupNameArgs { name: accepted })),
        Ok(GroupToolCall::Create { name, .. })
            if name == format!("{}x", "é".repeat(79)) && name.chars().count() == 80
    ));
    for rejected in ["e\u{301}".to_owned(), "e\u{301}".repeat(81)] {
        assert_eq!(
            group_call(&GroupsCommand::Create(GroupNameArgs { name: rejected }))
                .map_err(|error| error.code()),
            Err(ErrorCode::InvalidRequest)
        );
    }
